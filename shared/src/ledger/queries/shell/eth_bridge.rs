...
///
/// If a transfer hash is not featured in a response, it is
/// because it was either relayed to Ethereum, or because it
...
/// Given a list of keccak hashes, check whether they have been
/// relayed, expired or if they are still pending.
fn pending_eth_transfer_status<D, H>(
    ...
    H: 'static + StorageHasher + Sync,
{
    ...
    let merkle_tree = ctx
        .wl_storage
        .storage
        .get_merkle_tree(ctx.wl_storage.storage.get_last_block_height())
...
mod test_ethbridge_router {
    ...
    async fn test_bridge_pool_status() {
        ...
        // either been relayed or that it has expired. the rpc
        // respone shouldn't return its hash
        let mut transfer4 = transfer.clone();
        transfer4.transfer.amount = 3.into();

        // commit storage changes
        client.wl_storage.storage.block.height = 3.into();
        client.wl_storage.commit_block().expect("Test failed");
